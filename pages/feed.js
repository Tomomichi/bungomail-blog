...
export default class Sitemap extends React.Component {
  static async getInitialProps({ res }) {
    ...
    entries.forEach(post => {
      feed.addItem({
        title: post.title,
        id: `https://blog.bungomail.com/entry/${post.slug.replace(/(\d{4})(\d{2})(\d{2})(\d{6})/, '$1/$2/$3/$4')}`,
        link: `https://blog.bungomail.com/entry/${post.slug.replace(/(\d{4})(\d{2})(\d{2})(\d{6})/, '$1/$2/$3/$4')}`,
